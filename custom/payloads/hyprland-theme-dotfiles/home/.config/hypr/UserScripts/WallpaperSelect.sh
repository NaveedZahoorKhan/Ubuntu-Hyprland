...
USER_SCRIPTSDIR="$HOME/.config/hypr/UserScripts"
...
kill_wallpaper_for_video() {
  ...
  pkill mpvpaper-holder 2>/dev/null
...
# Rofi command. This is a dmenu list; passing `-show` without a mode makes
# Rofi open the "unsure what to show" placeholder instead of the wallpaper list.
...
modify_startup_config() {
  ...
  portable_file="${selected_file/#$HOME/\$HOME}"
  sed -i '/# BEGIN wallpaper restore/,/# END wallpaper restore/d' "$startup_config"
  sed -i '/^[[:space:]]*exec-once[[:space:]]*=.*StartSwww\.sh[[:space:]]*$/d' "$startup_config"
  sed -i '/^[[:space:]]*exec-once[[:space:]]*=.*MpvpaperLive\.sh[[:space:]]*start.*$/d' "$startup_config"

  # Check if it's a live wallpaper (video)
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm|MP4|MKV|MOV|WEBM)$ ]]; then
    cat >>"$startup_config" <<EOF

# BEGIN wallpaper restore
exec-once = \$HOME/.config/hypr/UserScripts/MpvpaperLive.sh start "$portable_file"
# END wallpaper restore
EOF
...
# BEGIN wallpaper restore
exec-once = $HOME/.config/hypr/UserScripts/StartSwww.sh
# END wallpaper restore
EOF
...
apply_video_wallpaper() {
  local video_path="$1"
  local mpvpaper_helper="$USER_SCRIPTSDIR/MpvpaperLive.sh"

  video_path="$(realpath "$video_path")"

  if [[ -x "$mpvpaper_helper" ]]; then
    "$mpvpaper_helper" start "$video_path"
    return
  fi
  ...
  mpvpaper --fork --auto-pause --mpv-options "load-scripts=no no-audio loop panscan=1.0" ALL "$video_path"
}
